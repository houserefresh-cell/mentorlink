...
import { MENTOR_DISCOVERY_CATEGORIES } from "@/lib/subject-discovery";
type DirectoryAction="details"|"activities"|"inquiry"|"meeting";type ActiveInteraction={mentor:PublicMentor;action:DirectoryAction};const INITIAL_BATCH=8;
const OPEN_ACTIVITY_EVENT = "mentorlink:open-activity";
function openParentActivity(activityId:string){
 const hash=`#activity-${activityId}`;
 if(window.location.hash!==hash)window.history.pushState(null,"",hash);
 window.dispatchEvent(new CustomEvent(OPEN_ACTIVITY_EVENT,{detail:{activityId}}));
}
...
function DirectoryContent({mentors,expandableFilters=false,interactionMode="parent",mentorActivities={},queryString}:{mentors:PublicMentor[];expandableFilters?:boolean;interactionMode?:"parent"|"read-only";mentorActivities?:Record<string,MentorPublicActivity[]>;queryString:string}){
 ...
 const appliedSubjects=unique(params.getAll("subject").map((value)=>value.trim()).filter(Boolean));
 const appliedModes=unique(params.getAll("mode").filter((value)=>MEETING_MODES.includes(value as typeof MEETING_MODES[number])));
 const rawMin=parseOptionalAge(params.get("minAge")),rawMax=parseOptionalAge(params.get("maxAge")),validUrlRange=rawMin===null||rawMax===null||rawMin<=rawMax,appliedMin=validUrlRange?rawMin:null,appliedMax=validUrlRange?rawMax:null,explicit=params.get("search")==="1",discovery=params.get("discovery")==="1";
 ...
 const requestedLabel=appliedSubjects.length===1?appliedSubjects[0]:null;
 const relatedTerms=useMemo(()=>{if(!requestedLabel)return[];const normalized=requestedLabel.trim().toLocaleLowerCase("he");const category=MENTOR_DISCOVERY_CATEGORIES.find((item)=>item.subjects.some((subject)=>[subject.label,...(subject.searchTerms??[])].some((term)=>term.trim().toLocaleLowerCase("he")===normalized)));return category?.subjects.flatMap((subject)=>[subject.label,...(subject.searchTerms??[])])??[]},[requestedLabel]);
 const alternatives=useMemo(()=>matching.length?[]:mentors.map((mentor)=>({mentor,score:mentor.subjects.filter((subject)=>relatedTerms.some((term)=>term.trim().toLocaleLowerCase("he")===subject.trim().toLocaleLowerCase("he"))).length})).sort((a,b)=>b.score-a.score).slice(0,4).map((item)=>item.mentor),[matching.length,mentors,relatedTerms]);
...
function mentorCardTone(mentor: PublicMentor) {
  const ageText = mentor.ageGroups.join(" ");
  if (/גן|כיתה א|כיתה ב|א׳|ב׳/.test(ageText)) return { border: "border-amber-300", surface: "from-white via-amber-50/40 to-yellow-50", bar: "bg-amber-400", chip: "border-amber-200 bg-amber-50 text-amber-900" };
  if (/כיתה ג|כיתה ד|כיתה ה|כיתה ו|ג׳|ד׳|ה׳|ו׳/.test(ageText)) return { border: "border-emerald-300", surface: "from-white via-emerald-50/40 to-green-50", bar: "bg-emerald-500", chip: "border-emerald-200 bg-emerald-50 text-emerald-900" };
  return { border: "border-blue-300", surface: "from-white via-blue-50/40 to-cyan-50", bar: "bg-blue-600", chip: "border-blue-200 bg-blue-50 text-blue-900" };
}
function MentorCard({ mentor, activities, onOpen, readOnly }: { mentor: PublicMentor; activities: MentorPublicActivity[]; onOpen: (mentor: PublicMentor, action: DirectoryAction, origin: HTMLButtonElement) => void; readOnly: boolean }) {
  ...
  const tone = mentorCardTone(mentor);
  return (
    <article className={`relative flex h-full min-w-0 flex-col overflow-hidden rounded-[1.8rem] border-2 bg-gradient-to-b p-5 shadow-[0_20px_45px_-28px_rgba(15,23,42,.55)] transition duration-200 hover:-translate-y-1 hover:shadow-xl ${tone.border} ${tone.surface}`}>
      <div className={`absolute inset-x-0 top-0 h-1.5 ${tone.bar}`} />
      <div className="mt-2 flex min-w-0 items-center gap-3"><Avatar initial={initial} large={false} photoUrl={mentor.profilePhotoUrl} /><div className="min-w-0"><button type="button" onClick={(event) => onOpen(mentor, "details", event.currentTarget)} className="break-words text-right text-xl font-black leading-tight text-slate-950 underline decoration-slate-300 underline-offset-4 hover:text-blue-700">{mentor.displayName}</button>{mentor.city && <City city={mentor.city} />}</div></div>
      {mentor.ageGroups.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{mentor.ageGroups.slice(0, 3).map((group) => <span key={group} className={`rounded-full border px-2.5 py-1 text-xs font-black ${tone.chip}`}>{group}</span>)}</div>}
      ...
      {mentor.nextAvailability?.length ? <div className="mt-3 rounded-2xl bg-blue-50 p-3"><p className="text-xs font-black text-blue-800">המועד הקרוב הזמין</p><p className="mt-1 text-sm font-black text-slate-900">{formatAvailability(mentor.nextAvailability[0])}</p></div> : null}
      {shortIntroduction && <p className="mt-3 line-clamp-2 break-words text-sm leading-6 text-slate-600">{shortIntroduction}</p>}
      {activities.length>0&&<button type="button" onClick={(event)=>activities.length===1?openParentActivity(activities[0].id):onOpen(mentor,"activities",event.currentTarget)} aria-haspopup={activities.length===1?undefined:"dialog"} className="mt-3 min-h-11 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2 text-right text-sm font-black text-violet-800 transition hover:bg-violet-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-600">{activities.length} פעילויות של החונך — לצפייה ולהרשמה ←</button>}
      <div className="mt-auto grid grid-cols-2 gap-2 pt-4">
        ...
        <button type="button" onClick={(event) => onOpen(mentor, "details", event.currentTarget)} aria-haspopup="dialog" className="min-h-11 rounded-xl px-3 py-2 text-sm font-black text-slate-600 transition hover:bg-slate-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600">פרטים נוספים</button>
      </div>
    </article>
  );
}
function MentorDetailsDialog({ mentor, activities, onActivities, onMeeting, onClose }: { mentor: PublicMentor; activities: MentorPublicActivity[]; onActivities: () => void; onMeeting?: () => void; onClose: () => void }) {
...
function MentorActivitiesDialog({ mentor, activities, onClose }: { mentor: PublicMentor; activities: MentorPublicActivity[]; onClose: () => void }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  const titleId = useId();
  useEffect(() => { dialogRef.current?.showModal(); }, []);
  function openActivity(activityId: string) {
    dialogRef.current?.close();
    openParentActivity(activityId);
  }
  return (
    <dialog ref={dialogRef} dir="rtl" aria-labelledby={titleId} onClose={onClose} onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }} className="m-auto max-h-[90dvh] w-[min(calc(100%_-_2rem),38rem)] overflow-y-auto rounded-3xl border border-violet-200 bg-white p-0 text-right text-slate-950 shadow-2xl backdrop:bg-slate-950/55">
      <div className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-slate-100 bg-white/95 px-5 py-4 backdrop-blur"><div><p className="text-sm font-black text-violet-700">פעילויות החונך</p><h2 id={titleId} className="text-xl font-black">הפעילויות של {mentor.displayName}</h2></div><button type="button" onClick={() => dialogRef.current?.close()} aria-label="סגירת פעילויות החונך" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-slate-200 text-2xl text-slate-600 transition hover:bg-slate-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-600">×</button></div>
      <div className="grid gap-4 p-5 sm:p-6">
        {activities.map((activity) => (
          <article key={activity.id} className="rounded-2xl border-2 border-violet-100 bg-violet-50/60 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-black">{activity.title}</h3><p className="mt-1 font-bold text-slate-700">{activity.subjectName}{activity.nextStartAt ? ` · ${new Intl.DateTimeFormat("he-IL", { weekday: "long", day: "numeric", month: "long", hour: "2-digit", minute: "2-digit" }).format(new Date(activity.nextStartAt))}` : ""}</p></div><span className={`rounded-full px-3 py-1 text-sm font-black ${activity.registrationOpen ? "bg-emerald-100 text-emerald-900" : "bg-slate-200 text-slate-700"}`}>{activity.registrationOpen ? "פתוחה להרשמה" : "ההרשמה סגורה — ניתן לצפות"}</span></div>
            <button type="button" onClick={() => openActivity(activity.id)} className="mt-4 min-h-11 w-full rounded-xl bg-violet-700 px-4 py-2 font-black text-white transition hover:bg-violet-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-700">{activity.registrationOpen ? "פרטי הפעילות והרשמה" : "פרטי הפעילות"}</button>
          </article>
        ))}
      </div>
    </dialog>
...
function formatAvailability(slot: { startAt: string; meetingMode: string; durationMinutes: number }) { return `${new Intl.DateTimeFormat("he-IL", { weekday: "long", day: "numeric", month: "long", hour: "2-digit", minute: "2-digit", timeZone: "Asia/Jerusalem" }).format(new Date(slot.startAt))} · ${slot.meetingMode} · ${slot.durationMinutes} דקות`; }

function EmptyState({ hasMentors, requestedSubject, onReset }: { hasMentors: boolean; requestedSubject: string | null; onReset: () => void }) {
  return (
    <div className="mt-5 rounded-3xl border border-dashed border-slate-300 bg-white px-5 py-12 text-center shadow-sm sm:px-8">
      ...
      <h3 className="mt-4 text-xl font-black text-slate-950">{hasMentors ? requestedSubject ? `עדיין לא נמצא חונך בתחום „${requestedSubject}” באזור שלכם` : "לא נמצאו חונכים שמתאימים לחיפוש" : "עדיין אין חונכים שפורסמו"}</h3>

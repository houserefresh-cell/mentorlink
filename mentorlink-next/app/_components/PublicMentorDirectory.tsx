...
function MentorCard({ mentor, activities, onOpen, readOnly }: { mentor: PublicMentor; activities: MentorPublicActivity[]; onOpen: (mentor: PublicMentor, action: DirectoryAction, origin: HTMLButtonElement) => void; readOnly: boolean }) {
  ...
  return (
    <article className="flex h-full min-w-0 flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_10px_28px_-22px_rgba(15,23,42,0.55)] transition duration-200 hover:-translate-y-0.5 hover:border-blue-200">
      ...
      {activities.length>0&&<button type="button" onClick={(event)=>onOpen(mentor,"details",event.currentTarget)} aria-haspopup="dialog" className="mt-3 min-h-11 rounded-xl border border-violet-200 bg-violet-50 px-3 py-2 text-right text-sm font-black text-violet-800 transition hover:bg-violet-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-600">{activities.length} פעילויות של החונך — לצפייה ולהרשמה ←</button>}

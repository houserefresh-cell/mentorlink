  ...
  return (
    <dialog ref={dialogRef} dir="rtl" aria-labelledby={titleId} onClose={onClose} onClick={(event) => { if (event.target === event.currentTarget) event.currentTarget.close(); }} className="m-auto max-h-[90dvh] w-[min(calc(100%_-_2rem),34rem)] overflow-y-auto rounded-3xl border border-slate-200 bg-white p-0 text-right text-slate-950 shadow-2xl backdrop:bg-slate-950/55">
      ...
      <div className="p-5 sm:p-6">
        ...
        {activities.length>0&&<DetailsSection title="הפעילויות של החונך"><div className="grid gap-2">{activities.map(activity=><a key={activity.id} href={`#activity-${activity.id}`} onClick={(event)=>{event.preventDefault();dialogRef.current?.close();openParentActivity(activity.id)}} className="rounded-xl border border-violet-200 bg-violet-50 p-3 font-bold text-violet-900 transition hover:bg-violet-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-600"><span className="block font-black">{activity.title}</span><span className="text-sm">{activity.subjectName}{activity.nextStartAt?` · ${new Intl.DateTimeFormat("he-IL",{day:"numeric",month:"long"}).format(new Date(activity.nextStartAt))}`:""} · {activity.registrationOpen?"פתוחה להרשמה":"ההרשמה נסגרה"}</span><span className="mt-2 block text-sm font-black text-blue-700">{activity.registrationOpen?"מעבר לפעילות ולהרשמה ←":"מעבר לפרטי הפעילות ←"}</span></a>)}</div></DetailsSection>}
      </div>
    </dialog>
  );
}

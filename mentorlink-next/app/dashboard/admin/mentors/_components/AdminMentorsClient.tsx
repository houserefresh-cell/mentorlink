...
function QuickMentorAccount({onCreated}:{onCreated:()=>void}){
  const[firstName,setFirstName]=useState(""),[lastName,setLastName]=useState(""),[email,setEmail]=useState(""),[password,setPassword]=useState("111111"),[busy,setBusy]=useState(false),[message,setMessage]=useState("");
  ...
  return <div className="mt-5 border-t border-violet-200 pt-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><label className="grid gap-1 font-bold">שם פרטי<input value={firstName} onChange={e=>setFirstName(e.target.value)} className={input}/></label><label className="grid gap-1 font-bold">שם משפחה<input value={lastName} onChange={e=>setLastName(e.target.value)} className={input}/></label><label className="grid gap-1 font-bold">אימייל<input type="email" dir="ltr" value={email} onChange={e=>setEmail(e.target.value)} className={input+" text-left"}/></label><label className="grid gap-1 font-bold">סיסמה זמנית<PasswordInput ariaLabel="סיסמה זמנית" value={password} onChange={e=>setPassword(e.target.value)} className={input+" text-left"} /></label></div><div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={create} disabled={busy||!firstName.trim()||!email.trim()||password.length<6} className="rounded-xl bg-blue-700 px-6 py-3 font-black text-white disabled:bg-slate-300">{busy?"יוצר...":"יצירת החשבון"}</button><button type="button" onClick={()=>setPassword(temporaryPassword())} className="rounded-xl border border-slate-300 bg-white px-5 py-3 font-bold">יצירת סיסמה אחרת</button></div>{message&&<p role="status" className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 font-bold [overflow-wrap:anywhere]">{message}</p>}</div>
}
...
function DetailView({ mentor }: { mentor: Detail }) {
  ...
  async function controlAccount() {
    ...
    try {
      ...
      body: JSON.stringify({ action: accountAction, reason: accountReason, suspendedUntil, confirmation: deleteConfirmation, force: accountAction === "permanently_delete" }),
      });
      const body = await response.json();

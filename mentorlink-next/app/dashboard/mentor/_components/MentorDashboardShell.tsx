...
export default function MentorDashboardShell({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{let active=true;async function load(){
  ...
  const token=(await supabase.auth.getSession()).data.session?.access_token;if(token){const headers={Authorization:`Bearer ${token}`};const accountResponse=await fetch("/api/mentor-account-status",{headers,cache:"no-store"}).catch(()=>new Response(null,{status:503}));if(accountResponse.ok){const account=await accountResponse.json();if(account.status!=="active"){setAccountControl(account);if(active)setReady(true);return}}const safeFetch=(input:string,init:RequestInit)=>fetch(input,init).catch(()=>new Response(null,{status:503}));const responses=await Promise.all([safeFetch("/api/meeting-requests",{headers,cache:"no-store"}),safeFetch("/api/mentor-inquiries",{headers,cache:"no-store"}),safeFetch("/api/mentor-availability",{headers,cache:"no-store"}),safeFetch("/api/mentor-profile",{headers,cache:"no-store"}),safeFetch("/api/notifications",{headers,cache:"no-store"})]);const bodies=await Promise.all(responses.map((response)=>response.ok?response.json():Promise.resolve({})));const meetings=bodies[0].requests??[],inquiries=bodies[1].inquiries??[],notifications=bodies[4].notifications??[],now=Date.now();setSummary({
   ...
   activityRegistrations:notifications.filter((item:{title?:string;read_at?:string|null})=>!item.read_at&&["הרשמה חדשה לפעילות","הצטרפות לרשימת ההמתנה"].includes(item.title??"")).length,

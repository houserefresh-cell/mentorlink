...
export default function SchedulingAvailabilityPage() {
  ...
  async function addWindow() {
    ...
    setMessage(response.ok ? (editingId ? "חלון הזמינות עודכן." : "חלון הזמינות נוסף.") : "לא ניתן לשמור את חלון הזמינות.");
...
<label className="grid gap-2 font-bold">מתאריך (לא חובה)<input type="date" value={form.effectiveStartDate} onChange={(event) => setForm({ ...form, effectiveStartDate: event.target.value })} className="rounded-xl border p-3" /></label><label className="grid gap-2 font-bold">עד תאריך (לא חובה)<input type="date" value={form.effectiveEndDate} onChange={(event) => setForm({ ...form, effectiveEndDate: event.target.value })} className="rounded-xl border p-3" /></label>
        <fieldset className="sm:col-span-2"><legend className="font-bold">משכים נתמכים</legend><div className="mt-2 flex flex-wrap gap-2">{DURATIONS.map((duration) => <button type="button" key={duration} onClick={() => setForm({ ...form, durations: form.durations.includes(duration) ? form.durations.filter((item) => item !== duration) : [...form.durations, duration] })} className={`rounded-xl border px-4 py-2 font-bold ${form.durations.includes(duration) ? "bg-blue-700 text-white" : ""}`}>{duration} דקות</button>)}</div></fieldset>
        <button type="button" onClick={addWindow} className="min-h-12 rounded-xl bg-blue-700 font-black text-white sm:col-span-2">{editingId ? "עדכון חלון זמינות" : "הוספת חלון זמינות"}</button>
      </div>
      ...
      {message && <p role="status" className="mt-5 rounded-xl bg-blue-50 p-4 text-center font-bold">{message}</p>}
    </MentorPageShell>
  );
}

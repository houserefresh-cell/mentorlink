...
type FormState = {
  ...
  audienceScope: "all" | "community"; communityIds: string[];
};
...
const initialForm: FormState = {
  ...
  locationType: "", venueName: "", address: "", locationDetails: "", pickupEnabled: false, pickupOptions: [], pickupDetails: "", sessions: [emptySession()], audienceScope: "all", communityIds: [],
};
...
export function MentorActivityForm({ activityId, needsNewDate = false }: { activityId?: string; needsNewDate?: boolean }) {
  ...
  const [capabilities, setCapabilities] = useState<{ age: number | null; isAdult: boolean; maxActivityPrice: number | null }>({ age: null, isAdult: false, maxActivityPrice: 25 });
  ...
  const [newSubjectCustomCategory, setNewSubjectCustomCategory] = useState("");
  ...
  const [communities, setCommunities] = useState<Array<{ id: string; name: string; locality: string }>>([]);
  const [visibility, setVisibility] = useState<{ generalScope: "public" | "community_restricted"; parentApprovedScope: "public" | "community_restricted"; communityIds: string[]; isMinor: boolean } | null>(null);
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(async ({ data, error }) => {
      ...
      const [subjectsResponse, capabilitiesResponse, activityResponse, communitiesResponse, visibilityResponse] = await Promise.all([
        fetch("/api/mentor-subjects", { headers, cache: "no-store" }),
        fetch("/api/mentor-capabilities", { headers, cache: "no-store" }),
        activityId ? fetch(`/api/mentor-activities/${activityId}`, { headers, cache: "no-store" }) : Promise.resolve(null),
        fetch("/api/communities", { headers, cache: "no-store" }),
        fetch("/api/mentor-visibility", { headers, cache: "no-store" }),
      ]);
      const subjectsBody = await subjectsResponse.json().catch(() => ({}));
      const capabilitiesBody = await capabilitiesResponse.json().catch(() => ({}));
      const activityBody = activityResponse ? await activityResponse.json().catch(() => ({})) : null;
      const communitiesBody = await communitiesResponse.json().catch(() => ({}));
      const visibilityBody = await visibilityResponse.json().catch(() => ({}));
      if (!active) return;
      if (!subjectsResponse.ok || !capabilitiesResponse.ok || !communitiesResponse.ok || !visibilityResponse.ok || (activityResponse && !activityResponse.ok)) {
        setMessage({ type: "error", text: activityBody?.error ?? capabilitiesBody.error ?? subjectsBody.error ?? "לא ניתן לטעון את טופס הפעילות." });
      ...
      setCapabilities(capabilitiesBody.capabilities ?? { age: null, isAdult: false, maxActivityPrice: 25 });
      setCommunities(communitiesBody.communities ?? []); setVisibility(visibilityBody);
      if (!activityId && visibilityBody.generalScope === "community_restricted") {
        setForm((current) => ({ ...current, audienceScope: "community", communityIds: visibilityBody.communityIds ?? [] }));
      }
  ...
  const canPublicAudience = Boolean(visibility && visibility.generalScope === "public" && (!visibility.isMinor || visibility.parentApprovedScope === "public"));
  const audienceCommunities = visibility?.generalScope === "community_restricted" ? communities.filter((community) => visibility.communityIds.includes(community.id)) : communities;
  const missingForPublication = publicationMissing(form, capabilities);
  ...
  async function createSubject() {
    ...
    const response = await fetch("/api/mentor-subjects", {
      ...
      body: JSON.stringify({ name: newSubjectName, category: newSubjectCategory === "__new__" ? newSubjectCustomCategory : newSubjectCategory, addToProfile }),
    });
    ...
    if (!response.ok) setMessage({ type: "error", text: body.error ?? "לא ניתן להוסיף את המקצוע." });
      ...
      setNewSubjectName("");
      setNewSubjectCustomCategory("");
      if (newSubjectCategory === "__new__") setNewSubjectCategory(subject.category);
  ...
  return <section dir="rtl" className="mx-auto max-w-5xl pb-16">
    ...
    <div className="space-y-6" aria-busy={busy}>
      <Card color="blue" title="1. מקצוע הפעילות">
        ...
        {subjectMode === "mine" ? <label className="mt-4 grid gap-2 font-bold">בחירת מקצוע *<select disabled={!editable} value={form.subjectId} onChange={(event) => setForm({ ...form, subjectId: event.target.value })} className={input}><option value="">בחירה</option>{mySubjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></label> : <div className="mt-4 space-y-4"><label className="grid gap-2 font-bold">מקצוע מהמאגר המשותף *<select disabled={!editable} value={form.subjectId} onChange={(event) => setForm({ ...form, subjectId: event.target.value })} className={input}><option value="">בחירה או הוספה חדשה</option>{catalog.map((subject) => <option key={subject.id} value={subject.id}>{subject.name} · {subject.category}</option>)}</select></label><div className="grid gap-3 rounded-2xl border border-dashed border-blue-300 bg-white p-4 md:grid-cols-[1fr_2fr]"><div className="grid gap-2"><select disabled={!editable} value={newSubjectCategory} onChange={(event) => setNewSubjectCategory(event.target.value as SubjectCategory)} className={input}>{[...new Set<string>([...SUBJECT_CATEGORIES, ...catalog.map((subject) => subject.category)])].map((category) => <option key={category}>{category}</option>)}<option value="__new__">+ קטגוריה חדשה</option></select>{newSubjectCategory === "__new__" && <input disabled={!editable} value={newSubjectCustomCategory} onChange={(event) => setNewSubjectCustomCategory(event.target.value)} placeholder="שם הקטגוריה החדשה" maxLength={50} className={input} />}</div><input disabled={!editable} value={newSubjectName}
 onChange={(event) => setNewSubjectName(event.target.value)} placeholder="שם מקצוע חדש" maxLength={50} className={input} /><label className="flex items-center gap-2 md:col-span-2"><input type="checkbox" disabled={!editable} checked={addToProfile} onChange={(event) => setAddToProfile(event.target.checked)} />הוסף את המקצוע גם לפרופיל שלי</label><button type="button" disabled={!editable || addingSubject || !newSubjectName.trim() || (newSubjectCategory === "__new__" && !newSubjectCustomCategory.trim())} onClick={createSubject}
 className="rounded-xl bg-blue-700 px-5 py-3 font-bold text-white disabled:bg-slate-400 md:col-span-2">{addingSubject ? "מוסיף..." : "הוספת המקצוע ובחירתו"}</button></div></div>}
      </Card>
...
<Field label="ציוד (לא חובה)"><textarea disabled={!editable} value={form.equipment} onChange={(e) => setForm({ ...form, equipment: e.target.value })} className={input} /></Field><Field label="נגישות והתאמות (לא חובה)" wide><div className="flex flex-wrap gap-2">{ACCESSIBILITY.map(([value, label]) => <CheckPill key={value} disabled={!editable} checked={form.accessibilityOptions.includes(value)} label={label} onChange={() => { const options = value === "unknown" ? (form.accessibilityOptions.includes("unknown") ? [] : ["unknown"]) : toggle(form.accessibilityOptions.filter((item) => item !== "unknown"), value); setForm({ ...form, accessibilityOptions: options, accessibilityOther: value === "other" && form.accessibilityOptions.includes("other") ? "" : form.accessibilityOther }); }} />)}</div>{form.accessibilityOptions.includes("other") && <input disabled={!editable} value={form.accessibilityOther} onChange={(e) => setForm({ ...form, accessibilityOther: e.target.value })} maxLength={1000} placeholder="פירוט ההתאמות האחרות" className={input} />}</Field><Field label="מדיניות ביטול (לא חובה)" wide><textarea disabled={!editable} value={form.cancellationPolicy} onChange={(e) => setForm({ ...form, cancellationPolicy: e.target.value })} className={input} /></Field></div>
      ...
      <Card color="blue" title="5. קהל הפעילות">
        <Field label="למי הפעילות פתוחה? *"><select disabled={!editable} value={form.audienceScope} onChange={(e) => setForm({ ...form, audienceScope: e.target.value as "all" | "community", communityIds: [] })} className={input}><option value="all" disabled={!canPublicAudience}>🌐 ציבורי / כולם ב-MentorLink</option><option value="community">🔒 קהילה/קהילות שאבחר</option></select>{!canPublicAudience && <span className="text-sm text-amber-800">חשיפה ציבורית דורשת הרשאה מתאימה. ניתן לבקש הרחבת הרשאה דרך החשבון שלי.</span>}</Field>
        {form.audienceScope === "community" && <div className="mt-4"><p className="mb-2 font-bold">בחירת קהילות</p><div className="flex flex-wrap gap-2">{audienceCommunities.map((community) => <CheckPill key={community.id} disabled={!editable} checked={form.communityIds.includes(community.id)} label={`${community.name}${community.locality ? ` · ${community.locality}` : ""}`} onChange={() => setForm({ ...form, communityIds: toggle(form.communityIds, community.id) })} />)}</div>{!audienceCommunities.length && <p className="mt-2 text-sm font-bold text-amber-800">אין כרגע קהילות זמינות לבחירה.</p>}</div>}
      </Card>
...
function publicationMissing(value: FormState, capabilities: { isAdult: boolean; maxActivityPrice: number | null }) {
  ...
  if (value.audienceScope === "community" && !value.communityIds.length) missing.push("לפחות קהילה אחת לקהל הפעילות");
  ...
  if (!value.isFree && !capabilities.isAdult && Number(value.price) > 25) missing.push("מחיר עד 25 ₪ למשתתף לחונך מתחת לגיל 18");
...
function hebrewError(code?: string, _fallback?: string) { const messages: Record<string, string> = {
 PUBLISH_INCOMPLETE: "יש להשלים את כל שדות החובה לפני הפרסום.", NO_FUTURE_SESSION: "יש להוסיף לפחות מפגש עתידי אחד.", INVALID_DEADLINE: "מועד ההרשמה חייב להיות לפני המפגש הראשון.", ACTIVITY_CONFLICT: "המועד מתנגש בפעילות אחרת שלך.", MEETING_CONFLICT: "המועד מתנגש בפגישה מאושרת.", INVALID_PICKUP: "יש להשלים את פרטי האיסוף האחר.", SUBJECT_NOT_ACTIVE: "המקצוע שנבחר אינו זמין.", ACTIVITY_LOCKED_ACTIVE_REGISTRATIONS: "לפעילות זו כבר קיימות הרשמות ולכן פרטיה המרכזיים נעולים. ניתן לשלוח עדכון לנרשמים או לבטל את הפעילות.", ACTIVITY_NOT_EDITABLE: "לא ניתן לערוך פעילות במצב הנוכחי.", MINOR_ACTIVITY_PRICE_LIMIT: "לחונך מתחת לגיל 18 מחיר הפעילות מוגבל ל־25 ₪ למשתתף."
 }; return (code && messages[code]) || "לא ניתן לשמור את הפעילות כרגע."; }

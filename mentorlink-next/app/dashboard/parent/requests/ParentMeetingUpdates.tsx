...
type MeetingUpdate = {
  ...
  update_type: "cancellation" | "details" | "reschedule";
  body: string;
  subject: string;
  childName: string;
  created_at: string;
...
const labels = { cancellation: "התראה חשובה — ביטול פגישה", details: "פרטי המפגש עודכנו", reschedule: "הוצע מועד חדש" };
...
export default function ParentMeetingUpdates() {
  ...
  async function load() {
    const response = await fetch("/api/parent/meeting-updates", { headers: { Authorization: `Bearer ${await token()}` }, cache: "no-store" });

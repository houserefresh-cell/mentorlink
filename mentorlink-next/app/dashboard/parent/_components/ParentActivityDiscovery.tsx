...
type Activity = {
  id: string; title: string; description: string; subjectId: number | null; subjectName: string; subjectCategory: string | null; mentorName: string; mentorBookingId: string | null; city: string | null;
  ...
  registeredNames: string[]; waitlistedNames: string[];
...
export default function ParentActivityDiscovery({ mentors }: { mentors: PublicMentor[] }) {
  ...
  const subjectFilters = useMemo(() => [...new Set(searchParams.getAll("subject").map((value) => value.trim()).filter(Boolean))], [searchParams]);
  ...
  const visible = useMemo(() => { const filtered = subjectFilters.length ? activities.filter((activity) => subjectFilters.includes(activity.subjectName)) : activities; const child = children.find((item) => item.id === selectedChild); if (!child) return filtered; const interestIds = new Set(child.interests?.map((interest) => interest.subjectId) ?? []); const score = (activity: Activity) => Number(interestIds.has(activity.subjectId ?? -1)) * 4 + Number(Boolean(child.grade && activity.suitableGrades?.includes(child.grade))) * 2 + Number(!activity.suitableGrades?.length); return [...filtered].sort((left, right) => score(right) - score(left)); }, [activities, children, selectedChild, subjectFilters]);
  ...
  return <section id="marketplace" aria-labelledby="activities-title" className="mt-8 scroll-mt-24">
    ...
    {showAddChild && <Modal title="הוספת ילד/ה" onClose={() => setShowAddChild(false)}><p className="leading-7 text-slate-700">הוספת ילד מתבצעת בשני שלבים קצרים כדי לשמור את הפרטים וההעדפות בצורה מסודרת.</p><Link href="/dashboard/parent/preferences" className="mt-5 block rounded-xl bg-blue-700 p-3 text-center font-black text-white">מעבר להוספת ילד/ה</Link></Modal>}
...
function ActivityHero({ activity }: { activity: Activity }) {
  const meta = activityHeroMeta(activity.subjectCategory ?? "", activity.subjectName);
  return <div className={`relative grid min-h-44 place-items-center overflow-hidden bg-gradient-to-br ${meta.gradient}`}>
    {activity.imageUrl ? <img src={activity.imageUrl} alt={activity.imageAlt || activity.title} className="h-48 w-full object-cover" /> : <div className="flex h-48 w-full items-center justify-center px-5 text-center"><div><span aria-hidden className="text-6xl drop-shadow-sm">{meta.icon}</span><p className="mt-3 text-sm font-black text-slate-700">{activity.subjectCategory || "MentorLink"}</p><p className="text-xl font-black text-slate-950">{activity.subjectName}</p></div></div>}
    <span className="absolute left-3 top-3 rounded-2xl bg-white/90 p-1.5 shadow-md backdrop-blur"><img src="/mentorlink-logo.png" alt="MentorLink" className="h-8 w-auto object-contain" /></span>
  </div>;
}
function activityHeroMeta(category: string, subject: string) {
  const text = `${category} ${subject}`.toLocaleLowerCase("he");
  if (/כדורגל|כדורסל|ספורט|טניס|כדור/.test(text)) return { icon: "⚽", gradient: "from-amber-100 via-emerald-100 to-sky-100" };
  if (/מתמט|אנגלית|עברית|לימוד|מדע|היסטור/.test(text)) return { icon: "📚", gradient: "from-blue-100 via-cyan-50 to-amber-50" };
  if (/מוזיק|גיטר|פסנתר|שירה/.test(text)) return { icon: "🎵", gradient: "from-violet-100 via-pink-50 to-blue-100" };
  if (/אומנות|יצירה|ציור|עיצוב/.test(text)) return { icon: "🎨", gradient: "from-orange-100 via-rose-50 to-violet-100" };
  if (/טכנולוג|מחשב|תכנות|קוד|דיגיטל/.test(text)) return { icon: "💻", gradient: "from-cyan-100 via-blue-50 to-violet-100" };
  if (/שפה|ספרד|צרפת|ערבית/.test(text)) return { icon: "💬", gradient: "from-rose-100 via-amber-50 to-cyan-100" };
  return { icon: "✨", gradient: "from-slate-100 via-blue-50 to-violet-100" };
}
function ActivityCard({ activity, action, onDetails, onRegister }:
 { activity: Activity; action: { label: string; enabled: boolean }; onDetails: () => void; onRegister: () => void }) { const first = activity.sessions[0]; return <article id={`activity-${activity.id}`} className="flex h-full scroll-mt-24 flex-col overflow-hidden rounded-3xl border border-blue-100 bg-white shadow-lg transition hover:-translate-y-1 hover:shadow-xl"><ActivityHero activity={activity} />
<div className="flex flex-1 flex-col p-5"><div className="flex items-start justify-between gap-3"><span className="rounded-full bg-violet-100 px-3 py-1 text-sm font-black text-violet-800">{activity.subjectName}</span><span className={`rounded-full px-3 py-1 text-sm font-black ${!activity.registrationOpen ? "bg-slate-200 text-slate-700" : activity.availablePlaces ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-900"}`}>{!activity.registrationOpen ? "ההרשמה נסגרה" : activity.availablePlaces ? `${activity.availablePlaces} מקומות פנויים` : "רשימת המתנה"}</span></div><h2 className="mt-4 text-2xl font-black">{activity.title}</h2><p className="mt-2 text-sm font-bold text-slate-500">בהובלת</p><MentorNameButton bookingId={activity.mentorBookingId} displayName={activity.mentorName} className="text-lg text-slate-950" /><p className="mt-3 rounded-xl bg-slate-50 p-3 text-sm font-bold text-slate-700"><span className="text-slate-500">קהל מתאים: </span>{audience(activity)}</p>
{activity.registeredNames.length>0&&<p className="mt-3 rounded-xl bg-violet-50 p-3 text-sm text-violet-950"><b>משתתפים:</b> {activity.registeredNames.join(" · ")}</p>}{activity.waitlistedNames.length>0&&<p className="mt-2 rounded-xl bg-amber-50 p-3 text-sm text-amber-950"><b>בהמתנה:</b> {activity.waitlistedNames.join(" · ")}</p>}{first && <div className="mt-4 rounded-2xl bg-blue-700 p-4 text-white"><p className="text-sm font-bold text-blue-100">המפגש הקרוב</p><p className="mt-1 text-lg font-black">{dateLabel(first.starts_at)}</p><p className="text-2xl font-black">{timeLabel(first.starts_at)}–{timeLabel(first.ends_at)}</p></div>}<div className="mt-4 grid grid-cols-2 gap-2 text-sm"><Info label="מיקום" value={activity.venueName || locationLabels[activity.locationType] || "יעודכן בהמשך"} /><Info label="מחיר" value={activity.isFree ? "ללא עלות" : `${activity.price} ₪`} /></div><div className="mt-auto grid grid-cols-2 gap-3 pt-5"><button onClick={onDetails} className="rounded-xl border border-blue-300 px-4 py-3 font-black text-blue-800">פרטים נוספים</button><button disabled={!action.enabled} onClick={onRegister} className="rounded-xl bg-green-600 px-4 py-3 font-black text-white disabled:bg-slate-300">{action.label}</button></div></div></article>; }
function ActivityDetails({ activity }: { activity: Activity }) { return <div className="grid gap-3 sm:grid-cols-2"><Detail title="תיאור" value={activity.description} wide /><Detail title="מועד" value={activity.sessions.map((session) => `${dateLabel(session.starts_at)}, ${timeLabel(session.starts_at)}–${timeLabel(session.ends_at)}`).join("\n")} wide /><div className="rounded-2xl border border-blue-100 bg-blue-50/60 p-4"><h3 className="font-black text-blue-900">מפעיל/ת הפעילות</h3><MentorNameButton bookingId={activity.mentorBookingId} displayName={activity.mentorName} className="mt-1 text-slate-800" /></div>
<Detail title="מיקום" value={[locationLabels[activity.locationType], activity.venueName, activity.locationDetails].filter(Boolean).join(" · ")} /><Detail title="קהל מתאים" value={audience(activity)} /><Detail title="משתתפים" value={`${activity.registeredCount} רשומים · ${activity.availablePlaces} פנויים מתוך ${activity.maxParticipants}`} /><Detail title="שמות המשתתפים" value={activity.registeredNames.join(" · ") || "אין עדיין משתתפים רשומים"} /><Detail title="רשימת המתנה" value={activity.waitlistedNames.join(" · ") || "אין ממתינים"} /><Detail title="ציוד" value={activity.equipment || "לא נדרש להביא ציוד"} /><Detail title="איסוף" value={activity.pickupOptions?.length ? `${activity.pickupOptions.join(", ")}${activity.pickupDetails ? ` · ${activity.pickupDetails}` : ""}` : "לא מוצע איסוף"} /><Detail title="נגישות והתאמות" value={[...(activity.accessibilityOptions ?? []).map((value) => accessibilityLabels[value] ?? value), activity.accessibilityOther].filter(Boolean).join(", ") || "לא סומנו התאמות נגישות"} /><Detail title="טלפון לחונך" value={activity.mentorPhone || (activity.contactPhoneVisibility === "registered_parents" ? "יוצג לאחר הרשמה מאושרת" : activity.contactPhoneVisibility === "mentor_approved" ? "יוצג לאחר אישור החונך" : "אינו מוצג")} /><Detail title="מדיניות ביטול" value={activity.cancellationPolicy || "לא צוינה מדיניות מיוחדת"} /><Detail title="מחיר" value={activity.isFree ? "ללא עלות" : `${activity.price} ₪`} /></div>; }

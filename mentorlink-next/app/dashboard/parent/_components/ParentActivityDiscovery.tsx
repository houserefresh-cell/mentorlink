...
function ActivityCard({ activity, onDetails, onRegister }: { activity: Activity; onDetails: () => void; onRegister: () => void }) { const first = activity.sessions[0]; return <article id={`activity-${activity.id}`} className="flex h-full scroll-mt-24 flex-col overflow-hidden rounded-3xl border border-blue-100 bg-white shadow-lg transition hover:-translate-y-1 hover:shadow-xl"><div className="grid min-h-40 place-items-center overflow-hidden bg-gradient-to-br from-cyan-100 via-blue-100 to-violet-100">{activity.imageUrl?<img src={activity.imageUrl} alt={activity.imageAlt||activity.title} className="h-44 w-full object-cover"/>:<ActivityIllustration small />}</div><div className="flex flex-1 flex-col p-5"><div className="flex items-start justify-between gap-3"><span className="rounded-full bg-violet-100 px-3 py-1 text-sm font-black text-violet-800">{activity.subjectName}</span><span className={`rounded-full px-3 py-1 text-sm font-black ${!activity.registrationOpen ? "bg-slate-200 text-slate-700" : activity.availablePlaces ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-900"}`}>{!activity.registrationOpen ? "ההרשמה נסגרה" : activity.availablePlaces ? `${activity.availablePlaces} מקומות פנויים` : "רשימת המתנה"}</span></div><h2 className="mt-4 text-2xl font-black">{activity.title}</h2><p className="mt-1 text-slate-600">עם {activity.mentorName}</p>{first && <div className="mt-4 rounded-2xl bg-blue-700 p-4 text-white"><p className="text-sm font-bold text-blue-100">המפגש הקרוב</p><p className="mt-1 text-lg font-black">{dateLabel(first.starts_at)}</p><p className="text-2xl font-black">{timeLabel(first.starts_at)}–{timeLabel(first.ends_at)}</p></div>}<div className="mt-4 grid grid-cols-2 gap-2 text-sm"><Info label="מיקום" value={activity.venueName || locationLabels[activity.locationType] || "יעודכן בהמשך"} /><Info label="מחיר" value={activity.isFree ? "ללא עלות" : `${activity.price} ₪`} /></div><div className="mt-auto grid grid-cols-2 gap-3 pt-5"><button onClick={onDetails} className="rounded-xl border border-blue-300 px-4 py-3 font-black text-blue-800">פרטים נוספים</button><button disabled={!activity.registrationOpen} onClick={onRegister} className="rounded-xl bg-green-600 px-4 py-3 font-black text-white disabled:bg-slate-300">{activity.registrationOpen ? "הרשמה" : "הרשמה סגורה"}</button></div></div></article>; }
function ActivityDetails({ activity }: { activity: Activity }) { return <div className="grid gap-3 sm:grid-cols-2"><Detail title="תיאור" value={activity.description} wide /><Detail title="מועד" value={activity.sessions.map((session) => `${dateLabel(session.starts_at)}, ${timeLabel(session.starts_at)}–${timeLabel(session.ends_at)}`).join("\n")} wide /><Detail title="מיקום" value={[locationLabels[activity.locationType], activity.venueName, activity.locationDetails].filter(Boolean).join(" · ")} /><Detail title="קהל מתאים" value={audience(activity)} /><Detail title="משתתפים" value={`${activity.minParticipants}–${activity.maxParticipants}`} /><Detail title="ציוד" value={activity.equipment || "לא נדרש להביא ציוד"} /><Detail title="איסוף" value={activity.pickupOptions?.length ? "קיימות אפשרויות איסוף" : "לא מוצע איסוף"} /><Detail title="נגישות והתאמות" value={activity.accessibilityOptions?.length ? activity.accessibilityOptions.map((value) => accessibilityLabels[value] ?? value).join(", ") : "לא סומנו התאמות נגישות"} /><Detail title="מדיניות ביטול" value={activity.cancellationPolicy || "לא צוינה מדיניות מיוחדת"} /><Detail title="מחיר" value={activity.isFree ? "ללא עלות" : `${activity.price} ₪`} /></div>; }

...
export default async function ParentHomePage(){let mentors:PublicMentor[]=[];try{mentors=await getPublishedMentors()}catch{console.error("Unable to load the parent mentor directory.")}return <div className="mx-auto max-w-7xl"><ParentActionSummary/><SubjectDiscovery targetPath="/dashboard/parent"/><ParentActivityDiscovery mentors={mentors}/><p className="mt-8 text-sm text-slate-500">בקשות שכבר שלחתם זמינות תמיד בעמוד <Link href="/dashboard/parent/requests" className="font-bold text-blue-700 underline">הבקשות והפגישות שלי</Link>.</p></div>}

...
export async function PATCH(request: Request, context: Context) {
  ...
  try {
    ...
    if (action === "cancel") {
      ...
      if (cancelled.error) {
        if (cancelled.error.message.includes("ACTIVITY_CANCELLATION_WITHIN_24_HOURS")) {
          return Response.json({ error: "נותרו פחות מ־24 שעות למפגש. יש ליצור קשר עם ההורים ועם מנהל המערכת לפני ביטול.", code: "ACTIVITY_CANCELLATION_WITHIN_24_HOURS" }, { status: 409 });
        }

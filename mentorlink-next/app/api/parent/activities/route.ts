...
export async function GET(request: Request) {
  ...
  const activities = await admin.from("mentor_activities").select("*").eq("status", "published").gt("registration_deadline", new Date().toISOString()).order("published_at", { ascending: false });
  ...
  const [sessions, registrations, profiles, subjects] = await Promise.all([
    ...
    subjectIds.length ? admin.from("subjects").select("id, name, category_id").in("id", subjectIds) : Promise.resolve({ data: [], error: null }),
  ]);
  if (sessions.error || registrations.error || profiles.error || subjects.error) return Response.json({ error: "לא ניתן להשלים את טעינת הפעילויות." }, { status: 500 });

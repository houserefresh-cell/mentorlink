...
async function childrenWithInterests(parentUserId: string) {
  ...
  const children = await admin.from("parent_children").select("id, first_name, grade, birth_date, school_name, accommodation_notes, created_at").eq("parent_user_id", parentUserId).order("created_at");

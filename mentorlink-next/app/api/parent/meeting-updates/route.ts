...
export async function GET(request: Request) {
  ...
  const meetings = meetingIds.length
    ? await admin.from("meeting_requests").select("id, subject, child_first_name").in("id", meetingIds).eq("parent_user_id", user.id)
    : { data: [], error: null };
  ...
  return Response.json({
    updates: (updates.data ?? []).map((update) => ({
      ...update,
      subject: meetingMap.get(update.meeting_request_id)?.subject ?? "פגישה",
      childName: meetingMap.get(update.meeting_request_id)?.child_first_name ?? "הילד/ה",
    })),

...
export async function POST(request: Request) {
  try {
    ...
    if (!/^\S+@\S+\.\S+$/.test(email) || password.length < 8 || !firstName) {
      return Response.json({ error: "יש להזין שם, אימייל תקין וסיסמה זמנית בת 8 תווים לפחות." }, { status: 400 });
    }
    ...
    if (created.error || !created.data.user) {
      ...
      return Response.json({ error: duplicate ? "כבר קיים חשבון עם כתובת המייל הזאת." : "לא ניתן ליצור את החשבון כרגע." }, { status: duplicate ? 409 : 500 });
    }
